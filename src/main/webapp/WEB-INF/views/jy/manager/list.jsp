...
function fn_redrawMemberList(list){
	...
	for(var i = 0; i < list.length; ++i){
		...
		td.append('    <td><a href="<%=context%>/mng/detail?mem_no=' + list[i].mem_no + '">' + list[i].mem_name +"</a></td>");
	...
	<table id="newTable">
		...
		<c:forEach var="member" items="${memberList }">
			<tr>
				...
				<td><a href="<%=context%>/mng/detail?mem_no=${member.mem_no }"> ${member.mem_name }</a></td>
